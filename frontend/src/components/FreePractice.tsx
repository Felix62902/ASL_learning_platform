...
// A list of words for the user to practice , "HELLO", "WORLD", "REACT", "SIGN"
const practiceWords = ["ABLE", "HELLO", "WORLD", "FAMILY", "GLASGOW"];
...
function FreePractice() {
  ...
  // Function to start a new word

  const startNewWord = () => {
    const newWord =
      practiceWords[Math.floor(Math.random() * practiceWords.length)];
    setWordToSpell(newWord);
    setCurrentCharacterIndex(0);
    setIsWordComplete(false);
    setIsCorrect(false);
    setPrediction("None");
    setIncompleteChar(newWord.split("")); // Split into characters
    setCompletedChar([]);
  };
  ...
  return (
    <div className="detector-container">
      {/* ... your existing JSX for webcam, canvas, loading messages ... */}
        ...
        <div className="practice-left-container">
          ...
          {isWordComplete ? (
            <div className="completion-message">
              <h3>Word Complete!</h3>
              <button className="nextword-button" onClick={startNewWord}>
                Practice Another Word
              </button>
            </div>
          ) : (
            <div className="prediction-display">
              {/* <h1>Current Word: {wordToSpell[currentCharacterIndex]}</h1> */}
